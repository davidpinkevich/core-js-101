...
function timeSpanToString(startDate, endDate) {
  ...
  if (ms < 100) newMs = `0${ms}`;
  if (ms < 10) newMs = `00${ms}`;
  return `${newHour}:${newMin}:${newSec}.${newMs}`;
}
...
function angleBetweenClockHands(date) {
  ...
  let newH = hour;
  if (hour > 12) newH = hour - 12;
  const degHour = 0.5 * (60 * newH + min);
  const degMin = 6 * min;
  const result = Math.abs(degHour - degMin);

  if (result > 180) return (Math.abs(180 - result) * Math.PI) / 180;
