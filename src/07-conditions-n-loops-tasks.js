...
function doRectanglesOverlap(rect1, rect2) {
  const rigthTop = rect1.top + rect1.height;
  const rigthLeft = rect1.left + rect1.width;

  if (rigthTop > rect2.top && rigthLeft > rect2.left) return true;
  return false;
}
...
function isInsideCircle(circle, point) {
  const first = (circle.center.x - point.x) * (circle.center.x - point.x);
  const second = (circle.center.y - point.y) * (circle.center.y - point.y);
  if (first + second < circle.radius ** 2) return true;
  return false;
}
...
function findFirstSingleChar(str) {
  const arr = str.split(' ').join('').split('').sort();
  const x = arr.find((numb, i, array) => array.indexOf(numb) === array.lastIndexOf(numb));
  if (!x) return null;
  return x;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${firstS}${first}, ${second}${lastS}`;
}
